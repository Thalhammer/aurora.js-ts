...
describe('core/stream', () => {
  ...
  it('float32', () => {
    ...
    // peeking big endian
    for (let i = 0; i < valuesBE.length; i++) expect(stream.peekFloat32(i * 4)).toBe(valuesBE[i]);

    // peeking little endian
    for (let i = 0; i < valuesLE.length; i++) expect(stream.peekFloat32(i * 4, true)).toBe(valuesLE[i]);

    // reading big endian
    for (let i = 0; i < valuesBE.length; i++) expect(stream.readFloat32()).toBe(valuesBE[i]);

    // reading little endian
    for (let i = 0; i < valuesLE.length; i++) expect(copy.readFloat32(true)).toBe(valuesLE[i]);

    // special cases
    let stream2 = makeStream([0xff, 0xff, 0x7f, 0x7f]);

    expect(stream2.peekFloat32(0)).toBeNaN();
    expect(stream2.peekFloat32(0, true)).toBe(3.4028234663852886e+38);
  });

  it('float64', () => {
    let stream = makeStream([0x55, 0x55, 0x55, 0x55, 0x55, 0x55], [0xd5, 0x3f]);
    let copy = stream.copy();
    expect(stream.peekFloat64(0)).toBe(1.1945305291680097e+103);
    expect(stream.peekFloat64(0, true)).toBe(0.3333333333333333);
    expect(stream.readFloat64()).toBe(1.1945305291680097e+103);
    expect(copy.readFloat64(true)).toBe(0.3333333333333333);

    stream = makeStream([1, 0, 0, 0, 0, 0], [0xf0, 0x3f]);
    copy = stream.copy();
    expect(stream.peekFloat64(0)).toBe(7.291122019655968e-304);
    expect(stream.peekFloat64(0, true)).toBe(1.0000000000000002);
    expect(stream.readFloat64()).toBe(7.291122019655968e-304);
    expect(copy.readFloat64(true)).toBe(1.0000000000000002);

    stream = makeStream([2, 0, 0, 0, 0, 0], [0xf0, 0x3f]);
    copy = stream.copy();
    expect(stream.peekFloat64(0)).toBe(4.778309726801735e-299);
    expect(stream.peekFloat64(0, true)).toBe(1.0000000000000004);
    expect(stream.readFloat64()).toBe(4.778309726801735e-299);
    expect(copy.readFloat64(true)).toBe(1.0000000000000004);

    stream = makeStream([0xff, 0xff, 0xff, 0xff, 0xff, 0xff], [0x0f, 0x00]);
    copy = stream.copy();
    expect(stream.peekFloat64(0)).toBeNaN();
    expect(stream.peekFloat64(0, true)).toBe(2.225073858507201e-308);
    expect(stream.readFloat64()).toBeNaN();
    expect(copy.readFloat64(true)).toBe(2.225073858507201e-308);

    stream = makeStream([0xff, 0xff, 0xff, 0xff, 0xff, 0xff], [0xef, 0x7f]);
    copy = stream.copy();
    expect(stream.peekFloat64(0)).toBeNaN();
    expect(stream.peekFloat64(0, true)).toBe(1.7976931348623157e+308);
    expect(stream.readFloat64()).toBeNaN();
    expect(copy.readFloat64(true)).toBe(1.7976931348623157e+308);

    stream = makeStream([0, 0, 0, 0, 0, 0], [0xf0, 0x3f]);
    copy = stream.copy();
    expect(stream.peekFloat64(0)).toBe(3.03865e-319);
    expect(stream.peekFloat64(0, true)).toBe(1);
    expect(stream.readFloat64()).toBe(3.03865e-319);
    expect(copy.readFloat64(true)).toBe(1);

    stream = makeStream([0, 0, 0, 0, 0, 0], [0x10, 0]);
    copy = stream.copy();
    expect(stream.peekFloat64(0)).toBe(2.0237e-320);
    expect(stream.peekFloat64(0, true)).toBe(2.2250738585072014e-308);
    expect(stream.readFloat64()).toBe(2.0237e-320);
    expect(copy.readFloat64(true)).toBe(2.2250738585072014e-308);

    stream = makeStream([0, 0, 0, 0, 0, 0], [0, 0]);
    copy = stream.copy();
    expect(stream.peekFloat64(0)).toBe(0);
    expect(stream.peekFloat64(0, true)).toBe(0);
    expect(1 / stream.peekFloat64(0, true) < 0).toBeFalsy();
    expect(stream.readFloat64()).toBe(0);
    expect(copy.readFloat64(true)).toBe(0);

    stream = makeStream([0, 0, 0, 0, 0, 0], [0, 0x80]);
    copy = stream.copy();
    expect(stream.peekFloat64(0)).toBe(6.3e-322);
    expect(stream.peekFloat64(0, true)).toBe(0);
    expect(1 / stream.peekFloat64(0, true) < 0).toBeTruthy();
    expect(stream.readFloat64()).toBe(6.3e-322);
    expect(copy.readFloat64(true)).toBe(0);

    stream = makeStream([0, 0, 0, 0, 0, 0], [0xf0, 0x7f]);
    copy = stream.copy();
    expect(stream.peekFloat64(0)).toBe(3.0418e-319);
    expect(stream.peekFloat64(0, true)).toBe(Infinity);
    expect(stream.readFloat64()).toBe(3.0418e-319);
