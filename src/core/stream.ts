...
export class Stream {
	...
	copy(): Stream {
		const res = new Stream(this.list.copy());
	...
	readBuffer(length: number): Buffer {
		const res = Buffer.allocate(length);
		const to = res.data;
	...
	readSingleBuffer(length: number): Buffer {
		const res = this.list.first.slice(this.localOffset, length);
		this.advance(res.length);
		return res;
	}

	peekSingleBuffer(offset: number, length: number): Buffer {
		const res = this.list.first.slice(this.localOffset + offset, length);
		return res;
	}
	...
	private float64Fallback(): number {
		const low = this.uint32[0];
		const high = this.uint32[1];
		if (high === 0 || high === 0x80000000) {
			return 0.0;
		}

		const sign = 1 - (high >>> 31) * 2;
		let exp = (high >>> 20) & 0x7ff;
		const frac = high & 0xffff;
		...
		case 'utf-8':
			...
			while (offset < end && (b1 = this.peekUInt8(offset++)) !== nullEnd) {
				if ((b1 & 0x80) === 0) {
					...
					const b2 = (this.peekUInt8(offset++) & 0x3f);
					result += String.fromCharCode(((b1 & 0x1f) << 6) | b2);

					// two continuation (2048 to 55295 and 57344 to 65535)
				} else if ((b1 & 0xf0) === 0xe0) {
					const b2 = this.peekUInt8(offset++) & 0x3f;
					const b3 = this.peekUInt8(offset++) & 0x3f;
	...
	private float80() {
		const high = this.uint32[0];
		const low = this.uint32[1];
